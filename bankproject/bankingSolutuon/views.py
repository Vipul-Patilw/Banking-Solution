...
from bankingSolutuon.models import Balance, ChangePin, Login ,BankDetails
...
from bankingSolutuon.models import ChangePassword,Lock,Lock2, Profile
...
def forgot_password(request):
	if request.method =="POST":
		...
		send_mail(subject,message,settings.EMAIL_HOST_USER,user_obj.email,token)
		messages.success(request,"email is sent go to Gmail open link given and change the password")
		return redirect('/forgot-password')
...
def password_recovery(request, token):
	...
	if request.method =="POST":
			...
			if new_password != confirm_password:
				messages.error(request,'both should be eqaul')
				return redirect(f'/change-password/{token}/')
			...
			user_obj.save()
			return redirect('/home')
	return render(request, 'resatePassword.html')